...
from xgb.tuneXGB1 import classifier_tuneXGB, tune_MySVM_MyRF, classifier_tuneSingXGB
...
def run_method(params: dict, hyp_train_x, train_y, hyp_test_x, test_y , labels,
...
#  choose which methods, here 'ExgboostE' or 'ExgboostP' depends use euclidean xgboost on which space data
    if params['method'][:8] == 'Pxgboost' or params['method'][:8] == 'Exgboost':
        ...
        trees = make_tree(hyp_train_x, train_y, params, params["num_trees"])

        test_y_pred, test_y_probs = evaluate_forest(trees, hyp_test_x, labels, params)
        acc, f1_macro, f1_micro, f1_weight, precision, recall = get_score(test_y, test_y_pred)

    elif params['method'] == 'hsvm':
        acc, f1_macro, f1_micro, f1_weight, precision, recall = hyper_train_binaryclass(hyp_train_x, train_y, params)
    elif params['method'] == 'LinearHSVM':
        hyp_SVM = hsvm.LinearHSVM(params)
        hyp_SVM.fit(hyp_train_x, train_y)
        # pred_scores = hyp_SVM.decision_function(hyp_test_x)
        y_pred = hyp_SVM.predict(hyp_test_x)
        # roc_scores = roc_auc_score(test_y, pred_scores)
        # acc_scores = np.sum(y_pred == test_y) / len(test_y)
        # precision, recall, _ = precision_recall_curve(test_y, pred_scores)
        ...
        clf = RandomForestClassifier(max_depth=params['max_depth'], random_state=0,
                criterion=params['criterion'], min_impurity_decrease = params['min_impurity_decrease'])
        ...
        classifier.init_pram(kclass=len(labels), seed=params['seed'], train_x=hyp_train_x,
                             train_y=train_y, test_x=hyp_test_x, test_y=test_y,eval_x = eval_x, val_y=val_y)
...
def run_train(params: dict, params_file):
    ...
    for gassid in range(1, (params['data_num']+1)):
        ...
        for seedid in range(params['folds']):
            train_x, test_x, train_y, test_y = train_test_split(full_train_x, full_train_y, test_size=0.3,
            ...
            acc, f1_macro, f1_micro, f1_weight, precesion, recall = run_method(params, hyp_train_x, train_y, hyp_test_x, test_y , labels)
...
class classifier_hyperxgb:
    ...
    def train(self, config_dict1: Configuration, round= None) -> tuple[float, Any, Any, Any]:
        ...
        booster_custom = xgb.train({'num_class': self.kClasses,
                               ...
                               evals=[(self.dtest, 'ttest')])#, early_stopping_rounds= 10
